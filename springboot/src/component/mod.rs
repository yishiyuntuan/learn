...
use crate::{application::AppBuilder, error::AppError};
...
/// Plugin Reference
#[derive(Clone)]
pub struct PluginRef(Arc<dyn Starter>);

/// Defined plugin interface
#[async_trait]
pub trait Starter: Any + Send + Sync {
    ...
    /// Configures a name for the [`Starter`] which is primarily used for checking plugin
...
impl PluginRef {
    pub(crate) fn new<T: Starter>(plugin: T) -> Self {
        Self(Arc::new(plugin))
    }
}

impl Deref for PluginRef {
    type Target = dyn Starter;
